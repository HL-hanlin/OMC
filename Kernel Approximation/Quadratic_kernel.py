...
# -*- coding: utf-8 -*-
"""
Quadratic kernel approximination
"""
...
def baseline_eval_second(x, G, m):
    ...
    """
    return (1/m**0.5) * (0.001*np.dot(G, x)**2)
...
def plot(episode, d_, MSE_iid_, MSE_qmc_, MSE_orthog_, MSE_max_ang_):
    ...
    plt.title('Angular Kernel Approximation')
...
def main(d, d_):
    ...
    random_sample = [i/(3*sigma) for i in random_sample]
    ...
    for e in tqdm(range(episode), position=0, leave=True):
        ...
        true_value = np.dot(baseline_eval_second(x, V, m), 
                            baseline_eval_second(y, V, m))
    ...
    for n in range(len(N)):
        ...
        for e in tqdm(range(episode), position=0, leave=True):
            ...
            for i in range(epoch):
                ...
                iid_estimates[n, e, i] = np.dot(baseline_eval_second(x, V, N[n]),
                            baseline_eval_second(y, V, N[n]))
                qmc_estimates[n, e, i] = np.dot(baseline_eval_second(x, all_V_qmc[n][i], N[n]),
                            baseline_eval_second(y, all_V_qmc[n][i], N[n]))
                orthog_estimates[n, e, i] = np.dot(baseline_eval_second(x, V_orthog*norms, N[n]),
                            baseline_eval_second(y, V_orthog*norms, N[n]))
                
                particle_matrix = random_rotation(sample_V[n][i], V_orthog)
                max_ang_estimates[n, e, i] = np.dot(baseline_eval_second(x, particle_matrix*norms, N[n]),
                            baseline_eval_second(y, particle_matrix*norms, N[n]))
