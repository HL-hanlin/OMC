...
def plot(episode, d_, MSE_iid_, MSE_qmc_, MSE_orthog_, MSE_max_ang_):
    ...
    plt.title('Gaussian_kernel_Comparison(with iid)')
...
def main(d, d_):
    ...
    N = [d*i for i in range(1,d_+1)]  # number of samplings of random features
    episode = 100
    ...
    for e in tqdm(range(episode), position=0, leave=True):
        ...
        true_value = math.exp((-1)*(np.linalg.norm(x - y)**2)/(2))
    ...
    for n in range(len(N)):
        ...
        for e in tqdm(range(episode), position=0, leave=True):
            ...
            for i in range(epoch):
                ...
                iid_estimates[n, e, i] = np.dot(baseline_eval_gaussian(x, V, N[n]),
                            baseline_eval_gaussian(y, V, N[n]))
                qmc_estimates[n, e, i] = np.dot(baseline_eval_gaussian(x, all_V_qmc[n][i], N[n]),
                            baseline_eval_gaussian(y, all_V_qmc[n][i], N[n]))
                orthog_estimates[n, e, i] = np.dot(baseline_eval_gaussian(x, V_orthog*norms, N[n]),
                            baseline_eval_gaussian(y, V_orthog*norms, N[n]))
                
                particle_matrix = random_rotation(sample_V[n][i], V_orthog)
                max_ang_estimates[n, e, i] = np.dot(baseline_eval_gaussian(x, particle_matrix*norms, N[n]),
                            baseline_eval_gaussian(y, particle_matrix*norms, N[n]))
