...
def baseline_eval_ang(x, G, m):
    ...
    """
    return (1/m**0.5) * np.array([1 if i >=0 else -1 for i in np.dot(G, x)])
...
def plot(episode, d_, MSE_iid_, MSE_qmc_, MSE_orthog_, MSE_max_ang_):
    ...
    plt.title('Angular Kernel Comparison')
    ...
    for e in range(episode):
        while True:
            ...
            if np.all(x == np.zeros(d))==0 and np.all(y == np.zeros(d))==0:
                break
        true_value = 1 - 2*angle(x,y)/math.pi
    ...
    for n in range(len(N)):
        ...
        for e in tqdm(range(episode), position=0, leave=True):
            ...
            for i in range(epoch):
                ...
                iid_estimates[n, e, i] = np.dot(baseline_eval_ang(x, V, N[n]),
                            baseline_eval_ang(y, V, N[n]))
                qmc_estimates[n, e, i] = np.dot(baseline_eval_ang(x, all_V_qmc[n][i], N[n]),
                            baseline_eval_ang(y, all_V_qmc[n][i], N[n]))
                orthog_estimates[n, e, i] = np.dot(baseline_eval_ang(x, V_orthog*norms, N[n]),
                            baseline_eval_ang(y, V_orthog*norms, N[n]))
                
                particle_matrix = random_rotation(sample_V[n][i], V_orthog)
                max_ang_estimates[n, e, i] = np.dot(baseline_eval_ang(x, particle_matrix*norms, N[n]),
                            baseline_eval_ang(y, particle_matrix*norms, N[n]))
