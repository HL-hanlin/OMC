...
def baseline_eval_cauchy(x, G, m):
...
def generate_qmc_features(d, d_, epoch, all_halton):
    ...
    for n in [d*i for i in range(1,d_+1)]:
        ...
        for i in range(epoch):
            V_qmc.append(stats.laplace.ppf(all_halton[n*i: n*(i+1)]))
...
def plot(episode, d_, MSE_iid_, MSE_qmc_, MSE_orthog_, MSE_max_ang_):
    ...
    plt.title('Cauchy kernel Comparison')
...
def main(d, d_):
    ...
    for n in range(len(N)):
        ...
        for e in tqdm(range(episode), position=0, leave=True):
            ...
            for i in range(epoch):
                V = np.random.laplace(size = (N[n], d))
                V_orthog = orthgonalize(V)   #create N*d matrix with orthogonal blocks
                norms = np.linalg.norm(V, axis=1).reshape([N[n],1])
                
                #b = np.random.uniform(0,2*math.pi, N[n])
                iid_estimates[n, e, i] = np.dot(baseline_eval_cauchy(x, V, N[n]),
                            baseline_eval_cauchy(y, V, N[n]))
                qmc_estimates[n, e, i] = np.dot(baseline_eval_cauchy(x, all_V_qmc[n][i], N[n]),
                            baseline_eval_cauchy(y, all_V_qmc[n][i], N[n]))
                orthog_estimates[n, e, i] = np.dot(baseline_eval_cauchy(x, V_orthog*norms, N[n]),
                            baseline_eval_cauchy(y, V_orthog*norms, N[n]))
                
                particle_matrix = random_rotation(sample_V[n][i], V_orthog)
                max_ang_estimates[n, e, i] = np.dot(baseline_eval_cauchy(x, particle_matrix*norms, N[n]),
                            baseline_eval_cauchy(y, particle_matrix*norms, N[n]))
