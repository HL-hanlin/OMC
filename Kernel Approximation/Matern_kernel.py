...
def baseline_eval_t(x, G, m):
    ...
    '''
    left = np.cos(np.dot(G, x))
    right = np.sin(np.dot(G, x))
...
def generate_qmc_features(d, d_, epoch, all_halton):
    ...
    for n in [d*i for i in range(1,d_+1)]:
        ...
        for i in range(epoch):
            V_qmc.append(stats.t.ppf(all_halton[n*i: n*(i+1)], 7))
...
def plot(episode, d_, MSE_iid_, MSE_qmc_, MSE_orthog_, MSE_max_ang_):
    ...
    plt.title('Matern_kernel_Comparison(with iid)')
...
def main(d, d_):
    ...
    degree = 7 # can change to order degree. E.g. degree of 5 in the other experiment
    ...
    for e in tqdm(range(episode), position=0, leave=True):
        ...
        #V_orthog = gram_schmidt(V)   #create N*d matrix with orthogonal blocks
        #norms = np.linalg.norm(V, axis=1).reshape([m,1])
        true_value = ((2**(1-degree/2))/math.gamma(degree/2))*\
                        ((np.sqrt(2*degree/2)*np.linalg.norm(x-y))**
                         (degree/2))*scipy.special.kv(degree/2, np.sqrt(2*degree/2)*\
                                                      np.linalg.norm(x-y))
    ...
    for n in range(len(N)):
        ...
        for e in tqdm(range(episode), position=0, leave=True):
            ...
            for i in range(epoch):
                np.random.seed()
                V = np.random.standard_t(degree, (N[n],d))   #create N*d iid matrix
                V_orthog = orthgonalize(V)   #create N*d matrix with orthogonal blocks
                norms = np.linalg.norm(V, axis=1).reshape([N[n],1])
                
                iid_estimates[n, e, i] = np.dot(baseline_eval_t(x, V, N[n]),
                            baseline_eval_t(y, V, N[n]))
                qmc_estimates[n, e, i] = np.dot(baseline_eval_t(x, all_V_qmc[n][i], N[n]),
                            baseline_eval_t(y, all_V_qmc[n][i], N[n]))
                orthog_estimates[n, e, i] = np.dot(baseline_eval_t(x, V_orthog*norms, N[n]),
                            baseline_eval_t(y, V_orthog*norms, N[n]))
                
                particle_matrix = random_rotation(sample_V[n][i], V_orthog)
                max_ang_estimates[n, e, i] = np.dot(baseline_eval_t(x, particle_matrix*norms, N[n]),
                            baseline_eval_t(y, particle_matrix*norms, N[n]))
